...
interface Props {
    submissions: Paginated<SubmissionModel>;
}
...
export default function Index(props: Props) {
    ...
    const dataColumns = useMemo<MRT_ColumnDef<SubmissionModel>[]>(
        () => [
            {
                header: 'User',
            ...
            {
                header: 'Quest',
                accessorKey: 'quest.title'
            },
            {
                id : 'status',
                header: 'Status',
                accessorFn: (originalRow) => <p className={`${SubmissionStatusColor(originalRow.status)}`}>
                    {originalRow.status}
                </p>,
            },
            {
                id : 'is_accepted',
                header: 'Penerimaan',
                accessorFn: row => row.is_accepted ? 'Diterima' : 'Tidak',
            },
            {
                header: 'Batas Waktu',
    ...
    const data = useMemo<SubmissionModel[]>(() => submissions.data, [
        JSON.stringify(submissions.data),
    ]);
        ...
        renderRowActions: ({ row }) => (
            <div className="flex items-center justify-center gap-2">
                <MuiInertiaLinkButton
                    color="primary"
                    href={route('submission.show', row.original.id)}
                >
                    Show
                </MuiInertiaLinkButton>
            </div>
